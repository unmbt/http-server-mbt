...
- [ ] Step 2: Verify artifacts in `target/cabi/`: `hs_thin.dll`, `hs_thin.lib`, `hs_thin_static.lib`, `hs_full.dll`, `hs_full.lib`, `hs_full_static.lib`, header `http_server.h`.
- [ ] Step 3: Analyze source code of `c_abi/include/http_server.h`, `c_abi/thin/bridge.c`, `c_abi/full/bridge.c`, `c_abi/thin/abi.mbt`, `c_abi/full/abi.mbt`.
- [ ] Step 4: Formulate adversarial hypotheses and attack vectors.
- [ ] Step 5: Implement and run comprehensive adversarial C test harnesses against dynamic & static libraries (`hs_thin` and `hs_full`).
