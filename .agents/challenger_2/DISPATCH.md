...
1. Inspect PE export directories of `target/cabi/hs_thin.dll` and `target/cabi/hs_full.dll` using `dumpbin /EXPORTS` or `llvm-nm` / `nm`:
...
2. Inspect `target/cabi/hs_thin_static.lib` and `target/cabi/hs_thin.dll` for cryptographic symbols:
...
3. Challenge Static Library Linkage & Main Collision:
   - Write an independent standalone C test application defining `int main(void)` and linking against `target/cabi/hs_thin_static.lib`.
...
4. Challenge Dynamic Library Linkage:
   - Compile a consumer program dynamically linking against `target/cabi/hs_thin.lib` with `hs_thin.dll` in runtime directory. Verify clean execution.
