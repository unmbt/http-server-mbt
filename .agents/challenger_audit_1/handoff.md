...
- **Target**: `http-server-mbt` (min & full CLI packaging, C ABI export pipeline)
- **Verdict**: **APPROVE**
...
2. **C ABI Robustness Harness (`target/adversarial_challenge.c` -> `target/cabi/adversarial_challenge.exe`)**:
   ...
   - Tested 7 test suites comprising 51 test cases:
     ...
     - 8 min-build unsupported features (`cert_file`, `key_file`, `key_passphrase`, `ca_file`, `proxy`, `proxy_all`, `proxy_options`, `websocket`) strictly rejected on `hs_min.dll` with `HS_ERR_UNSUPPORTED` (5).
...
3. **CLI Rejection (`cmd/http-server-min`)**:
   - Running `moon run cmd/http-server-min -- --cert test.crt`: exit code 1, stderr: `error: TLS is not supported in min build; use full build`.
   - Running `moon run cmd/http-server-min -- --key test.key`: exit code 1, stderr: `error: TLS is not supported in min build; use full build`.
   - Running `moon run cmd/http-server-min -- --proxy http://127.0.0.1`: exit code 1, stderr: `error: Proxy is not supported in min build; use full build`.
   - Running `moon run cmd/http-server-min -- -P http://127.0.0.1`: exit code 1, stderr: `error: Proxy is not supported in min build; use full build`.
   - Running `moon run cmd/http-server-min -- --proxy-all`: exit code 1, stderr: `error: Proxy is not supported in min build; use full build`.
   - Running `moon run cmd/http-server-min -- --proxy-config proxy.json`: exit code 1, stderr: `error: Proxy is not supported in min build; use full build`.
...
1. **Symbol Isolation**:
   ...
   - Therefore, R3 and Acceptance Criteria on symbol isolation and cryptographic decoupling for min are completely satisfied.
...
3. **CLI Argument Rejection**:
   - Observation 3 proves that all unsupported options (`--cert`, `--key`, `--key-passphrase`, `--proxy`, `-P`, `--proxy-all`, `--proxy-config`) cause `http-server-min` to terminate immediately with exit code 1 and actionable error messages prior to socket initialization.
...
- **Verdict**: **APPROVE**
- The implementation of the `min` and `full` CLI binaries and C ABI export pipeline satisfies all boundary robustness, error interception, symbol isolation, and regression criteria.
...
2. **Verify CLI Rejections**:
   ```powershell
   moon run cmd/http-server-min -- --cert test.crt # Exit code 1
   moon run cmd/http-server-min -- --proxy http://127.0.0.1:3000 # Exit code 1
   ```
