...
## Mission
Investigate CLI packaging and C ABI library export pipeline for min/full layered packaging, TLS decoupling, and Proxy architecture readiness.
...
- Milestone: min/full layered packaging, TLS decoupling, and Proxy architecture readiness
...
- **Key findings**:
  ...
  2. Modularizing into `cmd/http-server-min/`, `cmd/http-server-full/`, and shared `cmd/common/` allows clean decoupling while retaining `cmd/http-server-mbt/` as a backward-compatible alias.
  3. `min` CLI can enforce exit code 1 with descriptive error message on `--cert`/`--key`/`--proxy` before network listen.
...
## Key Decisions Made
- Recommended package split: `cmd/http-server-min/`, `cmd/http-server-full/`, and `cmd/common/`.
