...
1. Read `ORIGINAL_REQUEST.md` (especially latest `## Follow-up — 2026-09-18T13:08:02Z`) and `docs/cli-thin-full-and-cabi-handover.md`.
2. Explore `server/`, `full/`, `cmd/http-server-mbt-thin/`, `cmd/http-server-full/`, `core/`, `tls/`, `moon.mod.json`, `moon.pkg.json`.
...
4. Evaluate package structure options:
   - Should there be `c_abi/` with sub-packages `c_abi/thin` and `c_abi/full`, or `c_abi_min` and `c_abi_full` at root?
