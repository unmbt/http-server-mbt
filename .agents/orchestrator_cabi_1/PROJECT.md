# Project: http-server-mbt C ABI Export Pipeline (min & full)
...
- `target/cabi/`: Standard distribution output directory for exported libraries (`hs_min.dll`, `hs_min.lib`, `hs_min_static.lib`, `hs_full.dll`, `hs_full.lib`, `hs_full_static.lib`, `include/http_server.h`).
...
| 2 | C ABI Min Package (`c_abi/min`) | Pure static server bridge package; zero MbedTLS/crypto dependencies; maps JSON config to `server.with_server_at`; background OS owner thread | M1 | R1, explorer_codebase |
...
| 4 | Symbol Isolation & Def Files | `hs_min.def` and `hs_full.def` Windows module definition files guaranteeing DLL export table contains ONLY the 5 approved `hs_*` APIs; strips `.drectve` /EXPORT tags | M1 | R1, explorer_codebase, explorer_toolchain |
...
| 6 | Build Driver Pipeline (`scripts/build_cabi.mbtx`) | Pure `.mbtx` script using `@process` to drive `moon build`, compile bridge stubs, link DLLs (`hs_min.dll`, `hs_full.dll`), archive static LIBs (`hs_min_static.lib`, `hs_full_static.lib`), output to `target/cabi/` | M2 | R2, explorer_toolchain |
| 7 | C Consumer Dynamic Smoke Test | Standalone C program linking `hs_min.lib` / loading `hs_min.dll`, asserting `hs_abi_version() == 0x00010000` and testing server start/stop lifecycle | M3 | R3, spec_miner, explorer_toolchain |
| 8 | C Consumer Static Smoke Test | Standalone C program linking `hs_min_static.lib`, verifying static linkability without `main` symbol collision and verifying runtime execution | M3 | R3, explorer_toolchain |
| 9 | Symbol & Binary Purity Audit | Automated check verifying only `hs_*` exported in DLLs, 0 `mbedtls_*`/`psa_*` symbols in min artifacts, and 0 `main` collisions | M3 | R1, R2, D-11 |
...
| 1 | M1 C ABI Specification & Bridge Packages | Implement `c_abi/include/http_server.h`, `c_abi/min/`, `c_abi/full/`, `.def` files, and MoonBit bridge bindings | M0 | PLANNED |
