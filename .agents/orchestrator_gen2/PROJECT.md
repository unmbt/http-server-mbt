...
|---|------|-------|-------------|--------|
| M1 | Warning Elimination & Clean Baseline | Eliminate all 46 compiler warnings in `core`, `engine`, `server`, `cmd`, manifests; 0 warnings & 0 errors on `moon check --target native`. | None | **DONE** (Gate PASSED: 0 warnings, 0 errors, 6/6 tests pass) |
| M2 | Core Protocols, MIME, Security & Config | MIME registry, ETag/Range parsing, Basic Auth, path defense, BaseURL & SPA config validation. | M1 | **DONE** (Gate PASSED: 0 warnings, 0 errors, 30/30 tests pass) |
| M3 | Engine Features (HTTP/1.1, Compression, Directory, SPA) | StaticEngine request processing, pre-compression, HTML directory view, SPA/try-files fallback, file mutation abort. | M2 | **IN_PROGRESS** |
...
http-server-mbt/
├── core/
│   ├── config.mbt         # Config model, defaults, and pre-listen validation
│   ├── routing.mbt        # BaseURL normalization, alias resolution, boundary matching
│   ├── security.mbt       # Path traversal defense, Basic Auth, security headers
│   ├── mime.mbt           # MIME registry, .types parser, charset sniffing
│   ├── cache.mbt          # EntityTag, IMF-fixdate parser, 304 decision
│   ├── range.mbt          # Byte range parsing, Content-Range formatting
│   ├── core.mbt           # Core Request/Response types
