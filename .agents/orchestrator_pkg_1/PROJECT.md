# Project: thin & full layered packaging, TLS decoupling, and Proxy architecture readiness

## Architecture
- `core`: Pure HTTP data structures, MIME types, configuration, routing policies, proxy configurations. 100% TLS-free and I/O-free.
- `server`: Core HTTP/1.1 static file server, Range, conditional requests, compression negotiation, directory listing, and TransmitFile zero-copy. Decoupled from TLS via abstract `Transport` and `Acceptor` interfaces. 100% TLS-free in `thin` mode.
...
- `cmd/http-server-mbt-thin`: Minimal CLI executable. Does not compile MbedTLS. Fails with exit code 1 if `--cert`, `--key`, or `--proxy` options are specified.
...
### `cmd/common` ↔ `cmd/http-server-mbt-thin` / `cmd/http-server-full`
```moonbit
// In cmd/common:
...
- `cmd/http-server-mbt-thin/`: Minimal CLI executable
