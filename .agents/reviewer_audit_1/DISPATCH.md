...
## Objective
作为独立的代码与架构审查员（Independent Code & Architecture Reviewer），对 `http-server-mbt` 项目的 `min` 与 `full` 双版本解耦架构、C ABI 契约实现、以及 `.mbtx` 构建驱动进行全面、深入、逐行的代码与设计审查（R1）。
...
- 核心代码包:
  ...
  - `c_abi/min/` (含 `c_abi/min/moon.pkg`, `c_abi/min/c_abi_min.mbt`, `c_abi/min/bridge_min.c`, `c_abi/min/hs_min.def`)
  - `c_abi/full/` (含 `c_abi/full/moon.pkg`, `c_abi/full/c_abi_full.mbt`, `c_abi/full/bridge_full.c`, `c_abi/full/hs_full.def`)
  - `cmd/http-server-min/` 与 `cmd/http-server-full/`
...
- 审查 `cmd/http-server-min/` 与 `cmd/http-server-full/`：依赖拓扑是否符合精简与完整定义。
