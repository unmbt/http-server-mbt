...
- **Symbol Purity Verification**:
  - `dumpbin /EXPORTS target/cabi/hs_min.dll`: Exactly 5 exported functions (`hs_abi_version`, `hs_error_copy`, `hs_server_destroy`, `hs_server_start`, `hs_server_stop`), 0 `main`, 0 MoonBit runtime leaks.
...
- **Independent Execution Verification**:
  ...
  - `moon run scripts/build_cabi.mbtx`: Exit code 0, 6 artifacts generated (`hs_min.dll`, `hs_min.lib`, `hs_min_static.lib`, `hs_full.dll`, `hs_full.lib`, `hs_full_static.lib`), and all 4 C consumers passed:
...
- Static archive symbol check:
  ```powershell
  $out = & "E:/Program Files/msvc/VC/Tools/MSVC/14.42.34433/bin/HostX64/x64/dumpbin.exe" /SYMBOLS target/cabi/hs_min_static.lib; ($out | Select-String "mbedtls|psa_").Count
  ```
- CLI rejection flags:
  ```powershell
