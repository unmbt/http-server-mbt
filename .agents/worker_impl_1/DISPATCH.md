...
- **Task**: Implement C ABI export pipeline (min & full), .mbtx build driver script, and standalone C consumer smoke tests.
...
4. `E:/project/moonbit/unmbt/http-server-mbt/docs/cli-min-full-and-cabi-handover.md`

Your Objectives:
Implement the C ABI export pipeline (min & full), the .mbtx build driver script, and the standalone C consumer smoke tests:
...
1. **R1. C ABI Interface & Bridge Packages (`c_abi/`)**:
   ...
   - Create `c_abi/min/`:
     ...
     - `hs_min.def`: Windows module definition file listing ONLY the 5 `hs_*` exports.
   - Create `c_abi/full/`:
...
2. **R2. Pure `.mbtx` Build Pipeline (`scripts/build_cabi.mbtx`)**:
   - Write `scripts/build_cabi.mbtx` using `@process` from `moonbitlang/async/src/process`:
     ...
     - Generates `target/cabi/hs_min.dll` and `target/cabi/hs_min.lib` using `link.exe /DLL /DEF:c_abi/min/hs_min.def ...` (or clang/lld-link/gcc).
     - Generates `target/cabi/hs_min_static.lib` with `main` symbol elimination (e.g. `/Dmain=moonbit_unused_main` or library packaging).
     ...
     - Verifies symbol purity: verifies ONLY `hs_*` symbols in DLL export table, and zero `mbedtls_*`/`psa_*` in min artifacts.

3. **R3. Independent C Consumer Smoke Tests (`testdata/c_consumer/`)**:
   - Create `testdata/c_consumer/`:
     - `test_dynamic_min.c`: Links against `hs_min.lib`, runs with `hs_min.dll`, calls `hs_abi_version()`, starts server, stops server, destroys server.
     - `test_static_min.c`: Links against `hs_min_static.lib`, calls `hs_abi_version()`, starts server, stops server.
...
- No `mbedtls_*` or `psa_*` symbols in `min` artifacts.
