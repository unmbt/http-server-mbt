...
## Current Status
- Completed R1: C ABI interface header `c_abi/include/http_server.h`, `c_abi/thin/` package, `c_abi/full/` package, module definition export files (`hs_thin.def`, `hs_full.def`).
...
- Completed R3: Standalone C consumer smoke test programs (`testdata/c_consumer/`):
  - `test_dynamic_thin.c` (linked against `hs_thin.lib`, running with `hs_thin.dll`): PASS (exit code 0)
  - `test_static_thin.c` (linked against `hs_thin_static.lib`): PASS (exit code 0)
...
- Verified symbol isolation:
  - `dumpbin /EXPORTS target/cabi/hs_thin.dll`: exactly 5 functions (`hs_abi_version`, `hs_server_start`, `hs_server_stop`, `hs_server_destroy`, `hs_error_copy`), 0 `main`, 0 `moonbit_*` leaks.
  ...
  - `dumpbin /SYMBOLS target/cabi/hs_thin_static.lib`: 0 occurrences of `mbedtls` or `psa_` symbols.
