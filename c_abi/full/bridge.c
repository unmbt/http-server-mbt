...
extern void hs_full_run_server(int64_t server_ptr);
...
static int32_t parse_full_json_config(hs_server_t* s, const char* json, size_t len) {
    ...
    while (1) {
        ...
        if (strcmp(key, "root") == 0) {
            ...
            if (!parse_string_token(&p, s->cert_file, sizeof(s->cert_file))) return HS_ERR_CONFIG;
        } else if (strcmp(key, "key_file") == 0) {
            if (!parse_string_token(&p, s->key_file, sizeof(s->key_file))) return HS_ERR_CONFIG;
        } else if (strcmp(key, "key_passphrase") == 0) {
            if (!parse_string_token(&p, s->key_passphrase, sizeof(s->key_passphrase))) return HS_ERR_CONFIG;
        } else if (strcmp(key, "proxy") == 0) {
            if (!parse_string_token(&p, s->proxy, sizeof(s->proxy))) return HS_ERR_CONFIG;
        } else if (strcmp(key, "proxy_all") == 0) {
            if (!parse_string_token(&p, s->proxy_all, sizeof(s->proxy_all))) return HS_ERR_CONFIG;
        } else {
    ...
    // Mutual exclusion: fallback and proxy
    int has_fallback = s->spa || (s->try_files[0] != '\0');
    int has_proxy = (s->proxy[0] != '\0') || (s->proxy_all[0] != '\0');
    if (has_fallback && has_proxy) {
        return HS_ERR_CONFIG;
    }

    // proxy_all requires proxy
    if (s->proxy_all[0] != '\0' && s->proxy[0] == '\0') {
        return HS_ERR_CONFIG;
    }

    // TLS pair check
    if ((s->cert_file[0] != '\0' && s->key_file[0] == '\0') ||
        (s->cert_file[0] == '\0' && s->key_file[0] != '\0')) {
        return HS_ERR_CONFIG;
    }
...
MOONBIT_FFI_EXPORT moonbit_string_t hs_bridge_get_cert_file(int64_t server_ptr) {
    hs_server_t* s = (hs_server_t*)(uintptr_t)server_ptr;
    return utf8_to_moonbit_str(s ? s->cert_file : "");
}

MOONBIT_FFI_EXPORT moonbit_string_t hs_bridge_get_key_file(int64_t server_ptr) {
    hs_server_t* s = (hs_server_t*)(uintptr_t)server_ptr;
    return utf8_to_moonbit_str(s ? s->key_file : "");
}

MOONBIT_FFI_EXPORT moonbit_string_t hs_bridge_get_key_passphrase(int64_t server_ptr) {
    hs_server_t* s = (hs_server_t*)(uintptr_t)server_ptr;
    return utf8_to_moonbit_str(s ? s->key_passphrase : "");
}

MOONBIT_FFI_EXPORT moonbit_string_t hs_bridge_get_proxy(int64_t server_ptr) {
    hs_server_t* s = (hs_server_t*)(uintptr_t)server_ptr;
    return utf8_to_moonbit_str(s ? s->proxy : "");
}

MOONBIT_FFI_EXPORT moonbit_string_t hs_bridge_get_proxy_all(int64_t server_ptr) {
    hs_server_t* s = (hs_server_t*)(uintptr_t)server_ptr;
    return utf8_to_moonbit_str(s ? s->proxy_all : "");
}
...
static DWORD WINAPI server_thread_proc(LPVOID param) {
    ...
    ensure_runtime_init();
    hs_full_run_server((int64_t)(uintptr_t)s);
    return 0;
}
#else
...
HS_EXPORT int32_t hs_server_start(const char* json_config, size_t config_len, hs_server_t** out_server) {
    ...
    *out_server = NULL;
    ...
    if (json_config != NULL && config_len > 0) {
        int32_t err = parse_full_json_config(s, json_config, config_len);
        if (err != HS_OK) {
            free(s);
            return err;
        }
