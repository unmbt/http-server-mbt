...
本机 Windows，Moon 0.1.20260920：`moon run scripts/check_automation.mbtx`、`moon check --target native --deny-warn`、`git diff --check` 通过。`moon test --target native --deny-warn` 曾 261/263，两条句柄计数断言各差 1；分别重跑相关测试文件 7/7、7/7 通过。完整测试结果以同一提交的 Actions job 为准，本机波动仍须单独追踪。当前工作流失败原因与修补保持在草稿 PR #2；gate 成功前 T-025 不勾选。
