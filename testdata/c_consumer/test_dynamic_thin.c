...
int main(void) {
    ...
    printf("[test_dynamic_thin] ABI version: 0x%08X\n", ver);
    assert(ver == 0x00010000);

    printf("[test_dynamic_thin] Testing hs_error_copy...\n");
    char err_buf[64];
    size_t err_len = hs_error_copy(HS_ERR_CONFIG, err_buf, sizeof(err_buf));
    printf("[test_dynamic_thin] Error msg: %s (len: %zu)\n", err_buf, err_len);
    assert(err_len > 0);
    ...
    printf("[test_dynamic_thin] Testing preflight validation...\n");
    ...
    printf("[test_dynamic_thin] Testing hs_server_start, hs_server_stop, hs_server_destroy...\n");
    ...
    printf("[test_dynamic_thin] hs_server_start returned: %d\n", start_rc);
    assert(start_rc == HS_OK);
    assert(server != NULL);

    int32_t stop_rc = hs_server_stop(server);
    printf("[test_dynamic_thin] hs_server_stop returned: %d\n", stop_rc);
    ...
    printf("[test_dynamic_thin] hs_server_destroy completed successfully.\n");
