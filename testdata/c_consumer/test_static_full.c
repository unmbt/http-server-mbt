...
int main(void) {
    ...
    printf("[test_static_full] ABI version: 0x%08X\n", ver);
    assert(ver == 0x00010000);

    printf("[test_static_full] Testing hs_error_copy...\n");
    char err_buf[64];
    size_t err_len = hs_error_copy(HS_ERR_UNSUPPORTED, err_buf, sizeof(err_buf));
    printf("[test_static_full] Error msg: %s (len: %zu)\n", err_buf, err_len);
    assert(err_len > 0);
    assert(strcmp(err_buf, "Unsupported operation") == 0);

    printf("[test_static_full] Testing TLS preflight checks...\n");
    ...
    printf("[test_static_full] Testing server lifecycle...\n");
    ...
    printf("[test_static_full] hs_server_start returned: %d\n", start_rc);
    assert(start_rc == HS_OK);
    assert(server != NULL);

    int32_t stop_rc = hs_server_stop(server);
    printf("[test_static_full] hs_server_stop returned: %d\n", stop_rc);
